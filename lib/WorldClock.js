...
/* Change fixedTime from String to moment object. Use renderClock to present time in clock per clock's timezone 09-Jul-2020
//
...
class WorldClock {
  ...
  * @param {dictionary} [details] - details of new clock display. See below.
  *
  * @param {string} [details.tz=Europe/London] - The timezone for the clock.
  * @param {number} [details.timeFormat=24] - Time format: 12 for 12-hour clock or 24 for 24-hour clock
  * @param {string} [details.displaySeconds=false] - Whether clock displays seconds or not. "True" = display seconds
  * @param {string} [details.blinkSeparators=false] - Whether hours/minutes/seconds separator blinks. "True" = cause the separator to blink
  * @param {moment object} [details.fixedTime=now] - if provided, sets clock time in UTC & disables clock from running
  ...
  **/
  ...
  constructor ($aSpan, details) {
    ...
    */
    ...
    /** The inner span for week day
    * @member {jQuerySingleSpan}
    * @private
    */
    this._$wd = $('<span />').addClass('d-block').addClass('ddr-worldclock-weekday');
    this._$span.append(this._$wd);
    /**
    ...
    */
    ...
    this._$span.append('<br>');  // put timezone on a new line
    
    // the inner span for timezone name
    // optional?
    console.log (`WCC: add span for TZ name`);
    this._$tzname = $('<span />').addClass('d-block').addClass('ddr-worldclock-tzname');
...
/* ----------------------------------------------------------- */
  ...
  * @type {moment object}
  * @param {moment object} - clock time in UTC to use
   */
  set fixedTime(ftObject){
    //console.log (`SFT: enter set: ${ftObject}`);
    if(typeof ftObject === 'undefined'){
      //console.log (`SFT: ftObject is undefined. Use now.`);

      this._fixedTime = moment.utc().tz(this._timezone);    // get the current time in UTC
      //var now = moment('2016-12-19 21:30');
      
      this._fixedClock = false;  // set this instance as an updating clock
    }else{
      //console.log (`SFT: ftObject has input.`);

      // Validate input if correct format
      if (typeof ftObject != "object") {
        throw new TypeError('Fixed time for clock must be a valid moment object');
      }
      else if (!ftObject.isValid()) {
      
        throw new TypeError('Fixed time for clock must be a valid moment object: ' + ftObject.date());

      } else {
        this._fixedTime = ftObject;
        this._fixedClock = true;    // set this instance as a fixed time clock
        let lStr = ftObject.format('HH:mm');
        console.log (`SFT: making FT clock with time ${lStr}`);
      }
    }
  ...
  /* ----------------------------------------------------------- */
  /** 
  * Return WorldClock version string (static)
  * @type {string}
  */
  static get version(){
    return "WorldClock class version 1.7, modified 09/15/2020";
  }

  /* ----------------------------------------------------------- */
  /** 
  * Return Time format string for moment object (static)
  * @type {string}
  */
  ...
  static set TIME_FORMAT(l){
    throw new Error(`Unable to change moment time format string!`);
  }
  ...
  */
  renderClock(clock){
      ...
      if (clock._fixedClock) {
        console.log (`RC: rendering time for FT clock using ${clock._fixedTime.format(this.constructor.TIME_FORMAT)} and TZ ${clock._timezone}`);
        // if this is a fixed time clock, use time from fixedTime (which must be local)
        // how to specify input time is GMT, not local time?
        now = clock._fixedTime.tz(clock._timezone);  // get fixed time converted from UTC to local TZ
        let x = now.format("dddd HH:mm");
        //console.log (`RC: moment object has time ${x}`);
      ...
      // first add weekday
      clock._$wd.text(now.format('MMM-DD dddd')+" ");
